...
- **self-peptide space** (private) — computational immuno-oncology research in collaboration with a lab at the Icahn School of Medicine at Mount Sinai: can a tumor's mutated peptides be told apart from their healthy wild-type counterparts, and does that difference predict whether the immune system detects them? A rigorous, leak-free reanalysis lands a clear answer: how tightly a mutated peptide binds the immune system's presentation machinery does *not* predict whether it gets detected — the signal lives in how far the mutation pushes the peptide away from its healthy self, echoing how immunity flags altered-self. Detection turns out only weakly predictable from sequence, with negative results reported rather than buried
