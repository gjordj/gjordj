...
- **self-peptide space** (private) — computational immuno-oncology research in collaboration with a lab at the Icahn School of Medicine at Mount Sinai: can a tumor's mutated peptides be told apart from their healthy wild-type counterparts, and does that difference predict whether the immune system detects them? A rigorous, leak-free reanalysis lands a clear answer: among confirmed binders, how tightly a mutated peptide binds the immune system's presentation machinery does *not* predict whether it gets detected — and neither do the field's favored "altered-self" shortcuts (how foreign or how changed the mutation looks), each tested head-to-head and landing near chance. Immune detection turns out only weakly predictable from sequence — right at the ceiling of the best published models — independently reproducing the field's hard-won consensus that recognition takes both presentation and context, not any single score. Negative results reported rather than buried
